...
class JoePublicForm(ModelForm):
    ...
    province = forms.ChoiceField(
        choices=BLANK_CHOICE_DASH + list(PROVINCES_STATE_CHOICES),
...
class CreateJoePublicForm(JoePublicForm):
    ...
    same_name = forms.BooleanField(
        widget=HiddenInput(attrs={"value": False}), initial=False, required=False
    )
    ...
    def clean(self):
        ...
        if not same_name and len(anglers) > 0:
            ...
            msg = (
                "{} {} already exists. Do you want to use one of the "
...
class ReportForm(ModelForm):
    ...
    def clean_date_flag(self):
        ...
        date_flag = self.cleaned_data.get("date_flag", "")
...
class RecoveryForm(ModelForm):
    ...
    fate = forms.TypedChoiceField(
        choices=FATE_CHOICES_WITH_UNKN,
        initial="K",
        widget=forms.Select(attrs={"class": "form-control"}),
        required=False,
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "flen": forms.TextInput(
                attrs={"class": "form-control metric", "placeholder": "mm"}
            ),
            "tlen": forms.TextInput(
                attrs={"class": "form-control metric", "placeholder": "mm"}
            ),
            "girth": forms.TextInput(
                attrs={"class": "form-control metric", "placeholder": "mm"}
            ),
            "rwt": forms.TextInput(
                attrs={"class": "form-control metric", "placeholder": "grams"}
            ),
    ...
    def clean_clipc(self):
        ...
        clipc = self.cleaned_data.get("clipc")
        if clipc:
            clips = list(clipc)
    ...
    def clean_recovery_date(self):
        ...
        try:
            report_date = Report.objects.values_list("report_date").get(
                id=self.report_id
            )[0]
    ...
    def clean(self):
        ...
        latlon_flag = cleaned_data.get("latlon_flag", 0)

        if latlon_flag == "" or latlon_flag is None:
            latlon_flag = 0
        ...
        if flen and tlen:
            if flen > tlen:
                err_msg = (
                    "Total length (tlen) cannot be less than " "fork length (flen)."
                )
                raise forms.ValidationError(err_msg)
...
class EncounterUploadForm(forms.Form):
    """A simple little form for uploading our tempalte databases one at a time."""

    file_upload = forms.FileField(label="Encounter Data", required=True)

    def __init__(self, *args, **kwargs):

        super(EncounterUploadForm, self).__init__(*args, **kwargs)
