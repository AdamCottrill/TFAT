...
class TestAnlgerList(TestCase):
    ...
    def setUp(self):
        ...
        self.angler2 = JoePublicFactory.create(
            first_name="Burns", last_name="Montgomery"
        )

        self.angler3 = JoePublicFactory.create(first_name="Barney", last_name="Gumble")

        self.report_date = datetime(2010, 10, 10).replace(tzinfo=pytz.UTC)
    ...
    def test_anglers_render_in_angler_listview(self):
        ...
        response = self.client.get(reverse("tfat:angler_list"), follow=True)
    ...
    def test_angler_reports(self):
        ...
        response = self.client.get(
            reverse("tfat:angler_reports", kwargs={"angler_id": self.angler1.id}),
            follow=True,
        )

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "tfat/angler_reports.html")
        msg = "Tag Reports Filed By {} {}".format(
            self.angler1.first_name, self.angler1.last_name
        )
    ...
    def test_angler_reports_without_recoveries(self):
        ...
        """
        response = self.client.get(
            reverse("tfat:angler_reports", kwargs={"angler_id": self.angler3.id}),
            follow=True,
        )
        ...
        self.assertTemplateUsed(response, "tfat/angler_reports.html")
        msg = "Tag Reports Filed By {} {}".format(
            self.angler3.first_name, self.angler3.last_name
        )
        self.assertContains(response, msg)
