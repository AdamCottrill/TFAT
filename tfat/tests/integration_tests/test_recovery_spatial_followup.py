...
def test_recovery_spatial_follup_url(client, db_setup):
    ...
    response = client.get(reverse('spatial_followup'))
...
def test_recovery_spatial_followup_empty(client, db_setup):
    ...
    msg = ('There are currently no tags requiring spatial follow-up.' +
           ' Congratulations!')
...
def test_recovery_spatial_followup(client, db_setup):
    ...
    url = reverse('recovery_detail', kwargs={'recovery_id':recovery.id})
