...
#create map from Google Maps
mapImageData1 <- get_map(location = c(lon = center_pt[1],
                                      lat = center_pt[2]),
                         zoom = ZOOM,
                         source = "google",
                         maptype = "roadmap",
...
map <- ggmap(mapImageData1,
             extent = "device",
             ylab = "Latitude",
             xlab = "Longitude",
             legend = "topright")

map <- map + geom_point(data = tagging_event,
                        aes(x = dd_lon, y = dd_lat),
                        color = "purple", shape = 16, size = 3) +
  geom_point(data = tag_report,
             aes(x = dd_lon, y = dd_lat), color = "red",
             shape = 17,  size = 3)

map <- ggplot_build(map)

```

```{r create_map, echo = FALSE, results ='hide', include = FALSE}

png(filename = "./_maps/foobar.png", width = 480, height = 480, units = "px",
    pointsize = 12, bg = "white", res = NA, restoreConsole = TRUE)
print(map, justify = "centered")

dev.off()

```

![Figure 1: Map indicating the original tag location (purple circle),
and the recapture location (red triangle).](./_maps/foobar.png)
